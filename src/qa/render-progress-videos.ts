...
const outputDir = path.resolve(process.cwd(), "out/progress");
...
const main = async () => {
  ...
  const entries = progressSuite.filter((entry) => requestedIds.size === 0 || requestedIds.has(entry.id));

  if (entries.length === 0) {
    console.error("No progress prompts matched the requested ids.");
    process.exit(1);
  }
  ...
  for (const entry of entries) {
    ...
    console.log(`[progress-video] ${entry.id} -> ${outFile}`);
  }

  console.log(`Progress videos written to ${outputDir}`);
};

main().catch((error) => {
  console.error("Failed to render progress videos:", error);
  process.exit(1);
});
