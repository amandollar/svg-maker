...
const outputDir = path.resolve(process.cwd(), "out/text-qa");
...
const main = async () => {
  ...
  const entries = regressionPromptSuite.filter((entry) => entry.category === "text" && (requestedIds.size === 0 || requestedIds.has(entry.id)));
  ...
  for (const entry of entries) {
    ...
    for (const frame of getFrames(spec)) {
      ...
      console.log(`[text-qa] ${entry.id} frame ${frame} -> ${outFile}`);
    }
  }

  console.log(`Text preview stills written to ${outputDir}`);
};

main().catch((error) => {
  console.error("Failed to render text previews:", error);
  process.exit(1);
});
