...
const outputDir = path.resolve(process.cwd(), "out/scene-qa-videos");
...
const main = async () => {
  ...
  const entries = regressionPromptSuite.filter((entry) => entry.category === "scene" && idsToRender.has(entry.id));
  ...
  for (const entry of entries) {
    ...
    console.log(`[scene-video-qa] ${entry.id} -> ${outFile}`);
  }

  console.log(`Scene QA videos written to ${outputDir}`);
};

main().catch((error) => {
  console.error("Failed to render scene QA videos:", error);
  process.exit(1);
});
